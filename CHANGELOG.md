...
### Added
- TER cost (/coste-ter, and a tile on Resumen): what the management fees cost a year, and what they add up to over the projection horizon. The TER is written by hand on the instruments screen — no broker publishes it — and an instrument without one is **excluded and named**, never assumed to charge nothing: a fund counted as free flatters the weighted average by exactly the amount nobody has measured, so the projected cost is stated as a floor. The weighted figure is by current market value, answering what is being paid now rather than what the plan would pay once it is met. The projected cost is a **counterfactual, not a drag**: a fund's price is already net of its TER, so subtracting the fee again inside the projection would double-count it. Instead the fee is added back — `(1 + ter)^(1/12)` a month, so twelve months compound to exactly one year of fee — and the fee-free twin advances **on the same drawn month, inside the same loop**, for both the planned and the off-plan pot. The cost is taken path by path before any sorting: subtracting one run's p90 from another's subtracts two order statistics, which is only the fee bill of that path when the ranks happen to line up. The counterfactual is off unless asked for, so the ~60 simulations behind a goal solve do not pay for it. A TER is entered as a percent and refused outside 0–5%: a fraction typed where a percent was expected is a hundredfold error that looks entirely plausible on screen.
- Opportunity cost (/coste-oportunidad): every real purchase replayed into the index (VWCE.DE) at the close of its own day, so the question "would I have more money if I had just bought the index" gets an answer instead of an opinion. Two readings of the same reconstruction — the difference in euros and the difference in MWR — plus a per-position breakdown that says which pick paid for itself and which one cost, including positions already sold in full. **Both sides pay the same fees**, so what is left measures selection rather than the cost of operating. The counterfactual never sells, so the real side is measured as final wealth: what is still held plus the net cash the sales returned. Purchases older than the benchmark's price history buy nothing in the counterfactual and are named, with the flows and the amount left out, rather than valued at an interpolated price; a held position with no usable price is excluded from both sides. The benchmark is a parameter of the projection, not a constant inside it. The screen states what the number does not include: the tax the real sales generated and the counterfactual never pays.
...
- The projection now runs 10000 simulations instead of 3000, measured rather than assumed: over the plan held, that is where the bad and central scenarios settle to about 1% of movement between seeds. The good scenario moves roughly four times more at any count, because a tail is estimated from far fewer of the drawn paths than the median, so the screen says so instead of leaving the widest number on the panel to be read as the firmest. It converges at the same rate as the other two — the window is not what limits it — and simply needs more than an order of magnitude more simulations to get there, which the ~60 runs behind every goal solve make expensive.
- The convergence run judges each percentile on its own, and measures spread as a standard deviation across seeds rather than a range. A range grows with the number of seeds by construction, which made two runs incomparable and made the tail look as though it had stopped responding to the simulation count at all. It has not: fitted over the whole grid at enough seeds, every percentile falls as 1/√N, the tail included. The rate is now fitted across all simulation counts rather than read off the two end rows, seeds default to 24, and below 16 the verdict is withheld instead of guessed — at 8 the fitted exponent for the good scenario read -0.26 where 48 read -0.52, and the run was reporting that noise as a finding.
