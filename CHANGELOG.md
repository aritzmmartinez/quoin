...
### Added
- TWR and MWR for the whole portfolio, side by side on Resumen — the pair the instrument detail has always shown, finally answering the same question about everything at once. Neither is an average of the per-instrument figures, because a return is not additive: the TWR links the sub-period returns between value points with each period's flow taken out, and the MWR is the XIRR of every trade as the bank account saw it, closed by the portfolio's current value. Fees sit inside the MWR and outside the TWR, which is the whole distinction — TWR measures how the assets did, MWR how the money did — and a line under the two says why contributing month after month makes them diverge instead of leaving the gap to be read as an error.
- pnpm twr:explain --around=YYYY-MM-DD opens a single link and shows what the portfolio was made of at both ends: every instrument with its quantity, its value, and the price implied by the two (value ÷ quantity), alongside the raw snapshots either side of the window with any close of twice the previous one flagged. An extreme link has two causes that need opposite fixes — a real move on a position of pennies is a denominator problem, a bad snapshot is a data problem — and a floor applied over the second hides the symptom while the wrong price still feeds every valuation on every screen. The per-instrument values are printed with their sum, which must match the link's own V_start and V_end, so the breakdown cannot quietly disagree with the figure it explains.
- pnpm twr:explain opens the TWR chain link by link, worst first: every sub-period with its denominator, the flow inside it and the ratio that went into the product, plus how much of the compound comes from links with almost no money in them. A compounded return cannot be audited as a single number, and the classic way one goes wrong is a sub-period starting from a near-zero portfolio value, where a few euros of price movement become a percentage that then multiplies the whole history. It reads the same function the projection folds, so the explanation cannot drift from the figure it explains.
- The XIRR solver is Newton-Raphson with bisection behind it, and it refuses rather than guesses. Newton alone wanders off on irregular flows and returns a confident number nobody can defend; when it leaves the domain or fails to converge, a bracketed bisection finishes the job. Three cases end in null and say so on screen instead of printing a figure: fewer than two flows, flows that all point the same way (a portfolio only ever bought into has no rate that discounts to zero), and a set no rate solves. Tested against the published Excel XIRR example, against flows chosen because Newton alone fails on them, and against the textbook multiple-IRR case.
- Both figures are always nominal, even with the real switch on. The flows are the euros that actually left the bank; deflating the value series without deflating them would report a real return against nominal money, so the card says which euros it is quoting when the rest of the screen is in today's purchasing power.
- Returns can be read in real terms. A nominal/real switch on Resumen and /realizado restates every past amount in the purchasing power of the most recent published month, so the hero, the evolution chart and every closed sale answer "did this actually buy more than what I paid with" rather than "is the number bigger". The preference is persisted like the theme, because it is a lens you set once, not something you link to.
- The contributed figure is deflated flow by flow, never as a total. Each purchase enters the AVCO fold already restated at its own month, so eighteen months of monthly contributions are eighteen different exchange rates into today's euros — deflating the finished total instead applies one month's index to money that arrived across years and flatters the result. Both lines of the evolution chart are restated, the invested line at each contribution's date and the value line at each price mark's, since restating one and not the other turns plain inflation into a gap between them.
- Next to the switch, in real mode, a discreet line says which euros are on screen and how stale the index behind them is: "en euros de julio de 2026 · IPC actualizado hace 3 días". Same shape as "Actualizado hace 3 días" on Cartera, and for the same reason — the reference month says what the numbers mean, the sync time says whether anybody has checked for a newer one.
- pnpm ipc:sync pulls the monthly consumer price index from INE's Tempus3 API — the national general index and Bizkaia's, national by default. The whole series arrives in one request and the sync is idempotent by month, so re-running it adds only what is new.
- A missing month is reported, never filled. Real mode goes unavailable for the whole view and names the months it lacks, because interpolating one invents a price level nobody measured, and a total mixing restated and un-restated euros is a wrong number that looks right. A contribution made after the last published month is left in nominal euros instead — that month has not been measured yet, and this month's purchase is already this month's money.
- ipc:sync refuses to write when INE rebases the index. Every level is stored with the reference year it is 100 in, so republishing the series against a new base is visible instead of landing next to the old levels and corrupting every ratio that spans the boundary — silently, and in the flattering direction. The refusal says what to do about it: pnpm ipc:sync --force-rebase replaces the affected series wholesale. The base year is read off the data rather than tracked separately, since it is by definition the year whose twelve months average 100.
- The month of a trade is taken in Spanish civil time, not UTC. Spain runs an hour ahead in winter and two in summer, so the last hour or two of every month belongs to the next month locally, by a different amount on either side of the DST switch — a purchase at 00:30 Madrid on 1 April deflated with March's index, and nothing else wrong anywhere. Tests pin both month boundaries and both switch days.
- Realised P&L broken down by sale: /realizado lists every closed sale with the cost it consumed, the result in euros and in percent, and how long the units were held, grouped by calendar year with a subtotal per year and a global total. Each sale is valued at the average cost in force at that instant, not today's — a later purchase at a different price does not rewrite what a past sale cost. The screen says plainly that this is AVCO and will not match the FIFO the foral return uses, since the same sale has two correct answers depending on which question is being asked.
- The per-sale breakdown and the portfolio's realised total now read one shared AVCO walk, so they agree by construction rather than by coincidence: computePositions and computeRealizedGains are two views of the same fold, and a test asserts their sum matches to the cent, including a partial sell, a sell crossing purchases at different prices, a sale with a commission and a position reopened after being closed. Two implementations of the same arithmetic is how the two start disagreeing.
